...
export async function POST(request: Request) {
  // Qualquer excecao nao prevista aqui (ex.: auth()/DB indisponivel) deve
  // virar uma resposta JSON diagnosticavel -- sem este envolvimento
  // externo, o Next.js retorna uma pagina de erro HTML generica (HTTP
  // 500) que o cliente nao consegue interpretar como JSON.
  try {
    let user;
    try {
    ...
    console.error("Erro inesperado em /api/import/apol/confirm:", error);
    return NextResponse.json(
      {
        error: "Erro inesperado no servidor ao confirmar a importação.",
        details: error instanceof Error ? error.message : String(error),
      },
      { status: 500 },
    );
