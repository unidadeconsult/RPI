...
export async function POST(request: Request) {
  // Qualquer excecao nao prevista aqui (ex.: auth()/DB indisponivel, PDF
  // esgotando memoria) deve virar uma resposta JSON diagnosticavel -- sem
  // este envolvimento externo, o Next.js retorna uma pagina de erro HTML
  // generica (HTTP 500) que o cliente nao consegue interpretar como JSON.
  try {
    try {
    ...
    console.error("Erro inesperado em /api/import/apol/preview:", error);
    return NextResponse.json(
      {
        error: "Erro inesperado no servidor ao pré-visualizar o arquivo.",
        details: error instanceof Error ? error.message : String(error),
      },
      { status: 500 },
    );
